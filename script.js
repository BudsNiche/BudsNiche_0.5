console.log("[BudsNiche] V20 Final Dark Ames loaded — V19 game hardware preserved");
...
const LIST_MATCH_URL = `${SUPABASE_URL}/functions/v1/match-list`;
const LIST_MATCH_TIMEOUT_MS = 12000;
...
function rankedScoreFromRank(rankNumber) {
  const rank = Number(rankNumber);
  if (!Number.isInteger(rank) || rank < 1 || rank > 100) return 0;
  return 101 - rank;
}

async function loadRankedListAnswers(dailyGameId) {
  const endpoint =
    `${SUPABASE_URL}/rest/v1/game_list_answers?select=id,daily_game_id,rank_number,answer,aliases&daily_game_id=eq.${encodeURIComponent(dailyGameId)}&rank_number=lte.100&order=rank_number.asc`;
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`Could not load ranked list (${response.status}): ${message}`);
  }

  const rows = await response.json();
  return rows
    .filter((row) => Number(row.rank_number) >= 1 && Number(row.rank_number) <= 100)
    .map((row) => ({
      id: row.id,
      rank_number: Number(row.rank_number),
      answer: String(row.answer || ""),
      aliases: Array.isArray(row.aliases) ? row.aliases.map(String) : [],
      score: rankedScoreFromRank(row.rank_number)
    }))
    .sort((a, b) => a.rank_number - b.rank_number);
}

function listCandidateNames(item) {
  return [item.answer, ...(item.aliases || [])].filter(Boolean);
}

function findExactListMatch(answer, listAnswers) {
  const clean = normalize(answer);
  if (!clean) return null;

  for (const item of listAnswers) {
    if (listCandidateNames(item).some((name) => normalize(name) === clean)) {
      return item;
    }
  }
  return null;
...
function findConservativeFuzzyListMatch(answer, listAnswers) {
  const clean = normalize(answer);
  ...
  for (const item of listAnswers) {
    for (const name of listCandidateNames(item)) {
      const candidate = normalize(name);
      if (!candidate) continue;
      const distance = levenshteinDistance(clean, candidate);
      const longest = Math.max(clean.length, candidate.length);
      const similarity = longest ? 1 - distance / longest : 0;
      const record = { item, similarity, distance };
  ...
  if (!best) return null;
  const clearlyAhead = !secondBest || best.similarity - secondBest.similarity >= 0.04;
  const typoSized = best.distance <= 2 || best.similarity >= 0.92;
  return best.similarity >= 0.86 && typoSized && clearlyAhead ? best.item : null;
}

async function askListMatcher(round, answers) {
  if (!answers.length) return [];

  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), LIST_MATCH_TIMEOUT_MS);

  try {
    const response = await fetch(LIST_MATCH_URL, {
      method: "POST",
      headers: {
        apikey: SUPABASE_PUBLISHABLE_KEY,
        "Content-Type": "application/json"
    ...
    if (!response.ok) {
      const message = await response.text();
      throw new Error(`List matcher returned ${response.status}: ${message}`);
    }

    const data = await response.json();
    if (!data?.ok || !Array.isArray(data.matches)) {
      throw new Error("List matcher returned an unexpected response.");
    }
...
async function judgeListAnswers(round, answers) {
  const output = new Array(answers.length).fill(null);
  const unresolved = [];

  answers.forEach((answer, index) => {
    const exact = findExactListMatch(answer, round.list_answers || []);
    if (exact) {
      output[index] = {
        label: answer.trim() || "(blank)",
        score: exact.score,
        rank: exact.rank_number,
        concept: exact.answer,
        reason: `#${exact.rank_number} on the published ranked list.`,
        confidence: 1,
        needs_review: false,
        source: "list_exact"
      };
    } else {
      unresolved.push({ answer, index });
    }
  });

  if (unresolved.length) {
    try {
      const aiMatches = await askListMatcher(round, unresolved.map((item) => item.answer));
      const pool = aiMatches.map((match) => ({ match, used: false }));

      unresolved.forEach(({ answer, index }) => {
        const clean = normalize(answer);
        let found = pool.find(
          (entry) => !entry.used && normalize(entry.match?.player_answer || "") === clean
        );
        if (!found) found = pool.find((entry) => !entry.used);
        if (found) found.used = true;

        const rank = Number(found?.match?.rank_number);
        const listed = Number.isInteger(rank)
          ? (round.list_answers || []).find((item) => item.rank_number === rank)
          : null;
        ...
        if (listed) {
          output[index] = {
            label: answer.trim() || "(blank)",
            score: listed.score,
            rank: listed.rank_number,
            concept: listed.answer,
            reason: found?.match?.reason || `Matched to #${listed.rank_number} on the ranked list.`,
            confidence: Number(found?.match?.confidence ?? 0),
            needs_review: Boolean(found?.match?.needs_review),
            source: found?.match?.source || "list_ai_match"
          };
        }
      });
    } catch (error) {
      console.error("AI list matcher unavailable; using conservative typo matching.", error);
    }
  }

  unresolved.forEach(({ answer, index }) => {
    if (output[index]) return;
    const fuzzy = findConservativeFuzzyListMatch(answer, round.list_answers || []);
    if (fuzzy) {
      output[index] = {
        label: answer.trim() || "(blank)",
        score: fuzzy.score,
        rank: fuzzy.rank_number,
        concept: fuzzy.answer,
        reason: `Matched as a likely typo of #${fuzzy.rank_number}.`,
        confidence: 0.85,
        needs_review: false,
        source: "list_fuzzy_backup"
      };
    } else {
      output[index] = {
        label: answer.trim() || "(blank)",
        ...
        rank: null,
        concept: null,
        reason: "Not matched to the published Top 100 list.",
        confidence: 1,
        needs_review: false,
        source: "list_not_found"
      };
    }
  });
...
async function loadDailyGame() {
  try {
    const endpoint =
      `${SUPABASE_URL}/rest/v1/daily_game?select=id,round_number,category,topic,round_type,prompt,list_title,list_timeframe,list_source&date=eq.${encodeURIComponent(todayKey)}&published=eq.true&order=round_number.asc`;
    ...
    if (!rows.length) {
      ...
      const gameRoundCountIntro = document.getElementById("gameRoundCountIntro");
      if (gameRoundCountIntro) gameRoundCountIntro.textContent = "No published";
      if (startGameButton) {
        startGameButton.disabled = true;
        startGameButton.textContent = "NO GAME PUBLISHED YET";
      }
      return;
    }

    rounds = await Promise.all(rows.map(async (row) => {
      const profile = scoringProfiles[String(row.topic || "").toLowerCase()] || {
        concepts: []
      };
      const roundType = row.round_type || "niche";
      let listAnswers = [];

      if (roundType === "list") {
        try {
          listAnswers = await loadRankedListAnswers(row.id);
        } catch (error) {
          console.error(`Could not load ranked answers for game ${row.id}`, error);
        }
      }
      ...
      return {
        ...
        list_timeframe: row.list_timeframe || "",
        list_source: row.list_source || "",
        list_answers: listAnswers,
        // These local concepts are backup scoring + legacy Top 4 display for Niche rounds.
    ...
    const gameRoundCountIntro = document.getElementById("gameRoundCountIntro");
    if (gameRoundCountIntro) {
      gameRoundCountIntro.textContent = `${rounds.length} ${rounds.length === 1 ? "round" : "rounds"}`;
    }
    const startGameButton = document.getElementById("startGameButton");
    if (startGameButton) {
      startGameButton.disabled = false;
      startGameButton.textContent = "START TODAY'S GAME →";
    }
    prepareRound();
    ...
    if (startGameButton) {
      startGameButton.disabled = true;
      startGameButton.textContent = "GAME UNAVAILABLE";
    }
  }
}
...
function prepareRound() {
  ...
  categoryLabel.textContent = round.round_type === "list"
    ? `${round.category} // RANKED`
    : round.category;
  topicLabel.textContent = round.prompt || round.topic;
...
function topFourAnswers(round) {
  if (round.round_type === "list") {
    return (round.list_answers || [])
      .slice()
      .sort((a, b) => a.rank_number - b.rank_number)
      .slice(0, 4)
      .map((item) => ({
...
async function finishRound() {
  ...
  const isRankedRound = round.round_type === "list";
  answerProgress.textContent = submitted.length
    ? (isRankedRound ? "Checking the ranked list..." : "AI Judge is scoring your answers...")
    : "No answers submitted.";

  roundResults.innerHTML = submitted.length
    ? (isRankedRound
        ? `<h4 class="round-summary-title">RANKED LIST // CHECKING...</h4><p class="poll-result">#1 = 100 points • #100 = 1 point • outside the published list = 0.</p>`
  ...
  if (submitted.length) {
    if (round.round_type === "list") {
      results = await judgeListAnswers(round, submitted);
    } else {
      try {
  ...
  const listMeta = [round.list_title || round.topic, round.list_timeframe, round.list_source]
    .filter(Boolean)
    .join(" • ");
  const judgeNote = round.round_type === "list"
    ? `<p class="poll-result">Ranked scoring: #1 = 100, #2 = 99 … #100 = 1. Anything outside the published list scores 0.${listMeta ? ` ${escapeHtml(listMeta)}` : ""}</p>`
    : (usedBackupScoring
        ...
        <div class="result-list">
          ${results.map((r) => `
            <div class="result-item">
              <span>${escapeHtml(r.label)}${r.duplicate ? " (duplicate)" : ""}${r.rank ? ` → #${r.rank}` : ""}</span>
        ...
        ${
          top.length
            ? `<ol class="top-answer-list">
                ${top.map((item) => `<li>${item.rank ? `#${item.rank} ` : ""}${escapeHtml(item.label)} — <strong>${item.score}</strong></li>`).join("")}
               </ol>`
...
// Separate game launch — V20 adds a real lobby before the timer starts.
...
function openGameZone() {
  ...
  if (gameHasStarted) {
    ...
    if (gameBody) gameBody.classList.remove("hidden");
  } else {
    if (gameIntro) gameIntro.classList.remove("hidden");
    if (gameBody) gameBody.classList.add("hidden");
  }
...
if (startGameButton && gameIntro && gameBody) {
  startGameButton.addEventListener("click", () => {
    if (!rounds.length || startGameButton.disabled) return;
...
if (topGameButton && gameZone) {
  topGameButton.addEventListener("click", openGameZone);
}
...
// Responsive off-canvas sidebar. On phones the actual desktop rails move into
// the drawer, so users can still see every side widget without duplicate IDs.
const menuToggle = document.getElementById("menuToggle");
const mobileDrawer = document.getElementById("mobileDrawer");
const drawerClose = document.getElementById("drawerClose");
const drawerRailHost = document.getElementById("drawerRailHost");
...
function closeMobileDrawer() {
  ...
  if (!document.body.classList.contains("game-mode")) document.body.style.overflow = "";
}

function syncSidebarPlacement() {
  if (!siteShell || !centerFeed || !leftRail || !rightRail || !drawerRailHost) return;

  if (mobileRailQuery.matches) {
    if (leftRail.parentElement !== drawerRailHost) drawerRailHost.appendChild(leftRail);
    if (rightRail.parentElement !== drawerRailHost) drawerRailHost.appendChild(rightRail);
  } else {
    if (leftRail.parentElement !== siteShell) siteShell.insertBefore(leftRail, centerFeed);
    if (rightRail.parentElement !== siteShell) siteShell.appendChild(rightRail);
    closeMobileDrawer();
  }
}
...
document.querySelectorAll("[data-drawer-close]").forEach((element) => {
  element.addEventListener("click", closeMobileDrawer);
});
mobileRailQuery.addEventListener?.("change", syncSidebarPlacement);
window.addEventListener("resize", syncSidebarPlacement, { passive: true });
...
console.log("[BudsNiche] V20 starting data loaders", todayKey);
